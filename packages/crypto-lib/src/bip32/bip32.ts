...
function createApi(secp256k1) {
  return {
    ...
    pointFromScalar: (d: Buffer, compress: boolean) => secp256k1.publicKeyCreate(d, compress),
    privateAdd: (d : Buffer, tweak: Buffer) => secp256k1.privateKeyTweakAdd(new Uint8Array(d), tweak),
    sign: (h: Buffer, d: Buffer) => secp256k1.ecdsaSign(h, d),
    verify: (h: Buffer, Q: Buffer, signature: Buffer) => secp256k1.ecdsaVerify(signature, h, Q),
  };
}
...
function createTinySecp256k1Interface(secp256k1) {
  return {
    ...
    sign: (h: Uint8Array, d: Uint8Array, e?: Uint8Array): Uint8Array => Uint8Array.from(secp256k1.ecdsaSign(h, d)),
    verify: (h: Uint8Array, Q: Uint8Array, signature: Uint8Array, strict?: boolean): boolean => secp256k1.ecdsaVerify(signature, h, Q),
  }
}

const ecc = createApi(api)
export const tinySecp256k1Interface = createTinySecp256k1Interface(api)
