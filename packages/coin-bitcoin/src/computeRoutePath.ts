...
export type RoutePathInscriptionData = {
    contentType: string
    body: string | Buffer
    revealAddr: string
    repeat: number
...
export type RoutePathInscriptionRequest = {
    commitTxPrevOutputList: PrevOutput[]
    commitFeeRate: number
    revealFeeRate: number
    inscriptionDataList: RoutePathInscriptionData[]
...
type RoutePathInscriptionTxCtxData = {
    privateKey: Buffer
    inscriptionScript: Buffer
    commitTxAddress: string
    commitTxAddressPkScript: Buffer
    hash: Buffer
    revealTxPrevOutput: RoutePathTxOut
    revealPkScript: Buffer
}

export class RoutePathInscriptionTool {
    network: bitcoin.Network = bitcoin.networks.bitcoin;
    inscriptionTxCtxDataList: RoutePathInscriptionTxCtxData[] = [];
    ...
    static newRoutePathInscriptionTool(network: bitcoin.Network, request: RoutePathInscriptionRequest) {
        const tool = new RoutePathInscriptionTool();
        ...
        request.inscriptionDataList.forEach(inscriptionData => {
            tool.inscriptionTxCtxDataList.push(createRoutePathInscriptionTxCtxData(network, inscriptionData, privateKey));
        });
    ...
    buildEmptyRevealTx(network: bitcoin.Network, revealFeeRate: number, inscriptionDataList: RoutePathInscriptionData[]) {
...
function createRoutePathInscriptionTxCtxData(network: bitcoin.Network, inscriptionData: RoutePathInscriptionData, privateKeyWif: string): RoutePathInscriptionTxCtxData {
...
export function computeRoutePath(network: bitcoin.Network, request: RoutePathInscriptionRequest) {
    const tool = RoutePathInscriptionTool.newRoutePathInscriptionTool(network, request);
